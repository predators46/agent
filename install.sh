...
setDokodemoDoorUnblockStreamingMediaOutbounds() {
	...
	if [[ -n "${setIP}" ]]; then
		...
		if [[ -f "${configPath}09_routing.json" ]]; then
			unInstallRouting streamingMedia-80
			unInstallRouting streamingMedia-443
...
setDokodemoDoorUnblockStreamingMediaInbounds() {
	...
	echoContent yellow "7.域名录入示例:netflix,disney,hulu\n"
...
EOF
		...
		if [[ -f "${configPath}09_routing.json" ]]; then
			...
			local routing
			routing=$(jq -r '.routing.rules += [{"source":[],"type":"field","inboundTag":["streamingMedia-80","streamingMedia-443"],"outboundTag":"direct"},{"domains":[\"geosite:${domainList//,/\",\"geosite:}\"],"type":"field","inboundTag":["streamingMedia-80","streamingMedia-443"],"outboundTag":"blackhole-out"}]' ${configPath}09_routing.json)
			echo "${routing}" | jq . >${configPath}09_routing.json
		else
			cat <<EOF >${configPath}09_routing.json
            {
